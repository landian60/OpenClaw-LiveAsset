...
describe("control UI routing", () => {
  ...
  it("points the LiveAssets sidebar item at /live-assets/", async () => {
    const app = mountApp("/chat");
    await app.updateComplete;

    const link = Array.from(app.querySelectorAll<HTMLAnchorElement>("a.nav-item")).find(
      (entry) => entry.textContent?.trim() === "LiveAssets",
    );
    expect(link).not.toBeNull();
    expect(link?.getAttribute("href")).toBe("/live-assets/");
  });
