...
import {
  isCronSessionKey,
  parseSessionKey,
  resolveAgentMainSessionOptions,
...
function agentsList(overrides?: Partial<AgentsListResult>): AgentsListResult {
  return {
    defaultId: "main",
    mainKey: "main",
    scope: "per-sender",
    agents: [{ id: "main" }, { id: "zhn", name: "zhn" }, { id: "experiment", name: "experiment" }],
    ...overrides,
  };
}
...
describe("resolveAgentMainSessionOptions", () => {
  it("pins each agent main session even when sessions list is empty", () => {
    expect(resolveAgentMainSessionOptions(null, agentsList())).toEqual([
      { key: "agent:main:main", displayName: "Main Session" },
      { key: "agent:zhn:main", displayName: "zhn" },
      { key: "agent:experiment:main", displayName: "experiment" },
    ]);
  });

  it("uses session labels when an agent main session already exists", () => {
    const sessions: SessionsListResult = {
      ts: 0,
      path: "(multiple)",
      count: 1,
      defaults: { model: null, modelProvider: null, contextTokens: null },
      sessions: [row({ key: "agent:zhn:main", label: "Workspace ZHN" })],
    };
    expect(resolveAgentMainSessionOptions(sessions, agentsList())).toEqual([
      { key: "agent:main:main", displayName: "Main Session" },
      { key: "agent:zhn:main", displayName: "Workspace ZHN" },
      { key: "agent:experiment:main", displayName: "experiment" },
    ]);
  });

  it("returns no pinned agent sessions in global scope", () => {
    expect(resolveAgentMainSessionOptions(null, agentsList({ scope: "global" }))).toEqual([]);
  });
});
