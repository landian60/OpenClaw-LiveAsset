...
export function renderTab(state: AppViewState, tab: Tab) {
  const href = tab === "assets" ? "/live-assets/" : pathForTab(tab, state.basePath);
  if (tab === "assets") {
    return html`
      <a href=${href} class="nav-item" title=${titleForTab(tab)}>
...
function resolveSessionOptions(
  ...
  for (const option of resolveAgentMainSessionOptions(sessions, agentsList)) {
...
function resolveAgentMainSessionLabel(
  agent: AgentsListResult["agents"][number],
  agentsList: AgentsListResult,
): string {
  if (agent.id === agentsList.defaultId) {
    return "Main Session";
  }
  return agent.name?.trim() || agent.identity?.name?.trim() || agent.id;
}

export function resolveAgentMainSessionOptions(
  sessions: SessionsListResult | null,
  agentsList: AgentsListResult | null | undefined,
): SessionOption[] {
  if (!agentsList || agentsList.scope === "global") {
    return [];
  }

  const mainKey = agentsList.mainKey?.trim() || "main";
  return (agentsList.agents ?? []).map((agent) => {
    const key = `agent:${agent.id}:${mainKey}`;
    const row = sessions?.sessions?.find((entry) => entry.key === key);
    return {
      key,
      displayName: row?.label?.trim() || resolveAgentMainSessionLabel(agent, agentsList),
    };
  });
}
