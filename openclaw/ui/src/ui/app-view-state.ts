...
export type LiveAssetKeywordTrace = {
  kw: string;
  hit: boolean;
};

export type LiveAssetConstraintTrace = {
  action: string;
  reason?: string;
  when?: string;
  whenParams?: string;
...
export type LiveAssetMatchTrace = {
  matched: boolean;
  query: string;
  assetId: string;
  version: number;
  trigger: {
    any: LiveAssetKeywordTrace[];
      all: LiveAssetKeywordTrace[];
      not: LiveAssetKeywordTrace[];
  };
  inputAugmentation: string;
  firedInputRules: string[];
  processConstraints: LiveAssetConstraintTrace[];
  outputRules: Array<{ check: string; rewrite: string }>;
  tools: string[];
};

export type LiveAssetRuntimeEvent = {
  kind: string;
  time: number;
  assetId?: string;
  tool?: string;
  ...
  userInput?: string;
  pendingTools?: string[];
  constraintDetails?: LiveAssetConstraintTrace[];
  rewritePrompt?: string;
  rewrittenText?: string;
};

export type LiveAssetRuntimeState = {
  key: string;
  ...
  activeConstraintDetails: LiveAssetConstraintTrace[];
  events: LiveAssetRuntimeEvent[];
};

export type LiveAssetPanelState = {
...
> &
  Pick<CronModelSuggestionsState, "cronModelSuggestions"> & {
    ...
    handleOpenCodeAssetSidebar: (assetId: string) => void;
