...
export async function initSessionState(params: {
  ...
  if (!isNewSession && freshEntry) {
    ...
    // For idle-expired sessions, capture the old entry so that session_end
    // hooks fire and the old transcript is archived (same as explicit /new).
    if (!previousSessionEntry && entry) {
  ...
  // For idle-expired sessions (not triggered by /new or /reset), fire the
  // internal session:expired hook so that session-memory and other internal
  // hooks can save the old conversation context to memory files.
  if (isNewSession && !resetTriggered && previousSessionEntry?.sessionId) {
    void triggerInternalHook(
      createInternalHookEvent("session", "expired", sessionKey, {
        sessionEntry,
