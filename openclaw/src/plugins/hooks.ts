...
import type {
  PluginHookAssistantResponseEvent,
  PluginHookAssistantResponseResult,
...
export function createHookRunner(registry: PluginRegistry, options: HookRunnerOptions = {}) {
  ...
  /**
   * Run assistant_response hook.
   * Allows plugins to rewrite the final assistant text before payload assembly.
   * Handlers are executed sequentially and each one sees the latest rewritten text.
   */
  async function runAssistantResponse(
    event: PluginHookAssistantResponseEvent,
    ctx: PluginHookAgentContext,
  ): Promise<PluginHookAssistantResponseResult | undefined> {
    const hooks = getHooksForName(registry, "assistant_response");
    if (hooks.length === 0) {
      return undefined;
    }

    logger?.debug?.(`[hooks] running assistant_response (${hooks.length} handlers, sequential)`);

    let currentText = event.assistantText;
    let changed = false;
      ...
      try {
        const handlerResult = await (
          hook.handler as (
            event: PluginHookAssistantResponseEvent,
            ctx: PluginHookAgentContext,
          ) => Promise<PluginHookAssistantResponseResult | void>
        )(
          {
            ...event,
            assistantText: currentText,
            assistantTexts: currentText.trim() ? [currentText] : [],
          },
          ctx,
        );

        if (typeof handlerResult?.text === "string" && handlerResult.text !== currentText) {
          currentText = handlerResult.text;
          changed = true;
        }
      } catch (err) {
        handleHookError({
          hookName: "assistant_response",
    ...
    return changed ? { text: currentText } : undefined;
  }
