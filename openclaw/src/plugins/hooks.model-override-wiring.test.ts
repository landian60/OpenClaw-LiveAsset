 ...
 * 2. before_prompt_build receives session messages and can mutate prompt context
...
describe("model override pipeline wiring", () => {
  ...
  describe("before_prompt_build (attempt.ts pattern)", () => {
    ...
    it("hook can append context to the current prompt", async () => {
      const handlerSpy = vi.fn(
        (event: PluginHookBeforePromptBuildEvent) =>
          ({
            appendContext: `Append after ${event.prompt}`,
          }) as PluginHookBeforePromptBuildResult,
      );
      ...
      expect(handlerSpy).toHaveBeenCalledTimes(1);
      expect(result?.appendContext).toBe("Append after test");
    });
