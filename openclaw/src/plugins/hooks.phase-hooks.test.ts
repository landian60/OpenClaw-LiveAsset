...
describe("phase hooks merger", () => {
  ...
  it("before_prompt_build concatenates appendContext, prependSystemContext, and appendSystemContext", async () => {
    addTypedHook(
      registry,
      "before_prompt_build",
      "first",
      () => ({
        appendContext: "append prompt A",
    ...
    expect(result?.appendContext).toBe("append prompt A\n\nappend prompt B");
