...
afterEach(() => {
  ...
  hookRunnerMock.runAssistantResponse.mockReset();
});
...
describe("runEmbeddedPiAgent", () => {
  it("rewrites the final assistant response via assistant_response hook before returning", async () => {
    const sessionFile = nextSessionFile();
    const cfg = makeOpenAiConfig(["mock-1"]);
    hookRunnerMock.hasHooks.mockImplementation((hookName: string) => hookName === "assistant_response");
    hookRunnerMock.runAssistantResponse.mockResolvedValue({ text: "rewritten by hook" });
    ...
    const result = await runEmbeddedPiAgent({
      ...
      timeoutMs: 5_000,
      agentDir,
      runId: nextRunId("assistant-response"),
      enqueue: immediateEnqueue,
    });

    expect(hookRunnerMock.runAssistantResponse).toHaveBeenCalledWith(
      expect.objectContaining({ assistantText: "ok" }),
      expect.objectContaining({ sessionId: "session:test" }),
    );
    expect(result.payloads?.[0]?.text).toBe("rewritten by hook");

    const messages = await readSessionMessages(sessionFile);
    const lastAssistant = messages.at(-1);
    expect(lastAssistant?.role).toBe("assistant");
    expect(textFromContent(lastAssistant?.content)).toBe("rewritten by hook");
  });
