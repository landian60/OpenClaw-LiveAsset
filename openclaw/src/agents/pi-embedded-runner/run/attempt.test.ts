...
describe("resolvePromptBuildHookResult", () => {
  ...
  it("passes rawUserInput through to prompt-build and legacy hooks", async () => {
    const hookRunner = {
      hasHooks: vi.fn(() => true),
      runBeforePromptBuild: vi.fn(async () => undefined),
...
describe("applyPromptBuildHookContext", () => {
  it("appends prompt context after the current user prompt", () => {
    expect(
      applyPromptBuildHookContext({
        prompt: "current user utterance",
        appendContext: "live-assets input control",
      }),
    ).toBe("current user utterance\n\nlive-assets input control");
  });

  it("keeps prepend and append ordering deterministic", () => {
    expect(
      applyPromptBuildHookContext({
        prompt: "current user utterance",
