...
import {
  resolveAssistantResponseText,
  rewriteLastAssistantInSessionManager,
  rewriteLastAssistantMessageInPlace,
} from "../../assistant-response-rewrite.js";
import {
...
type PromptBuildHookRunner = {
  ...
  runBeforePromptBuild: (
    event: { prompt: string; rawUserInput?: string; messages: unknown[] },
...
export function applyPromptBuildHookContext(params: {
  prompt: string;
  prependContext?: string;
  appendContext?: string;
}): string {
  let effectivePrompt = params.prompt;
  if (params.prependContext) {
    effectivePrompt = `${params.prependContext}\n\n${effectivePrompt}`;
  }
  if (params.appendContext) {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        {
          if (hookResult?.prependContext || hookResult?.appendContext) {
            effectivePrompt = applyPromptBuildHookContext({
              prompt: params.prompt,
      ...
      const assistantResponseText = resolveAssistantResponseText({
        assistantTexts,
        lastAssistant,
      });
      if (hookRunner?.hasHooks("assistant_response") && assistantResponseText) {
        const rewritten = await hookRunner.runAssistantResponse(
          {
            runId: params.runId,
            sessionId: params.sessionId,
            provider: params.provider,
        ...
        if (typeof rewritten?.text === "string" && rewritten.text !== assistantResponseText) {
          const rewrittenText = rewritten.text;
          const rewrittenSnapshotAssistant = rewriteLastAssistantMessageInPlace(
            messagesSnapshot,
            rewrittenText,
          );
          const rewrittenActiveAssistant = rewriteLastAssistantMessageInPlace(
            activeSession.messages,
            rewrittenText,
          );
          const persisted = rewriteLastAssistantInSessionManager(sessionManager, rewrittenText);

          if (!rewrittenSnapshotAssistant && !rewrittenActiveAssistant) {
            log.warn(
              `assistant_response rewrite had no assistant message to replace: runId=${params.runId} sessionId=${params.sessionId}`,
            );
          }
          if (!persisted) {
            log.warn(
              `assistant_response rewrite was not persisted to the session transcript: runId=${params.runId} sessionId=${params.sessionId}`,
            );
          }

          assistantTexts.length = 0;
          if (rewrittenText) {
            assistantTexts.push(rewrittenText);
          }
          lastAssistant = rewrittenSnapshotAssistant ?? rewrittenActiveAssistant ?? lastAssistant;
        }
      }
