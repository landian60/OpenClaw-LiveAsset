...
const CONTROL_UI_BUILD_INPUT_PATHS = [
  ["ui", "index.html"],
  ["ui", "package.json"],
  ["ui", "vite.config.ts"],
  ["ui", "public"],
...
function isControlUiBuildInputFile(filePath: string): boolean {
  const base = path.basename(filePath);
  return !base.includes(".test.") && !base.includes(".spec.");
}

function newestMtimeMsForPathSync(targetPath: string): number {
  const stat = fs.statSync(targetPath);
  const ownMtimeMs = Number.isFinite(stat.mtimeMs) ? stat.mtimeMs : 0;
  if (!stat.isDirectory()) {
    return isControlUiBuildInputFile(targetPath) ? ownMtimeMs : 0;
  }
  let newestMtimeMs = 0;
  for (const entry of fs.readdirSync(targetPath, { withFileTypes: true })) {
    newestMtimeMs = Math.max(
      newestMtimeMs,
      newestMtimeMsForPathSync(path.join(targetPath, entry.name)),
    );
  }
  return newestMtimeMs;
}

function resolveControlUiSourceNewestMtimeMsSync(root: string): number {
  let newestMtimeMs = 0;
  for (const segments of CONTROL_UI_BUILD_INPUT_PATHS) {
    const candidate = path.join(root, ...segments);
    if (!fs.existsSync(candidate)) {
      continue;
    }
    newestMtimeMs = Math.max(newestMtimeMs, newestMtimeMsForPathSync(candidate));
  }
  return newestMtimeMs;
}

export function isControlUiDistStaleSync(root: string, indexPath: string): boolean {
  try {
    const distStat = fs.statSync(indexPath);
    const distMtimeMs = Number.isFinite(distStat.mtimeMs) ? distStat.mtimeMs : 0;
    return resolveControlUiSourceNewestMtimeMsSync(root) > distMtimeMs;
  } catch {
    return false;
  }
}
  ...
  return {
    ...
    stale: Boolean(
      indexPath &&
        ...
        (() => {
          const repoRoot =
            opts.root ?? resolveControlUiRepoRoot(opts.argv1 ?? process.argv[1]);
          return repoRoot ? isControlUiDistStaleSync(repoRoot, indexPath) : false;
        })(),
    ),
  };
}
...
export async function ensureControlUiAssetsBuilt(
  ...
  if (fs.existsSync(indexPath) && !isControlUiDistStaleSync(repoRoot, indexPath)) {
    return { ok: true, built: false };
  }
  ...
  runtime.log(
    health.stale
      ? "Control UI assets stale; rebuilding (ui:build, auto-installs UI deps)…"
