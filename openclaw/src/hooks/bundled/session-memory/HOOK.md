---
name: session-memory
description: "Save session context to memory when /new or /reset command is issued, or when a session expires due to idle timeout"
...
metadata:
  {
    "openclaw":
      {
        "emoji": "💾",
        "events": ["command:new", "command:reset", "session:expired"],
