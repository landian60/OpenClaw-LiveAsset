...
const saveSessionToMemory: HookHandler = async (event) => {
  // Trigger on reset/new commands OR idle-expired session transitions
  const isResetCommand =
    event.type === "command" && (event.action === "new" || event.action === "reset");
  const isSessionExpired = event.type === "session" && event.action === "expired";
  if (!isResetCommand && !isSessionExpired) {
    return;
  }
