...
type AgentKeyResolutionContext = {
  cfg: OpenClawConfig;
  defaultAgentId: string;
  knownAgentIds: Set<string>;
};

type KnownAgentAlias = {
  agentId: string;
  rest: string;
};

function createAgentKeyResolutionContext(cfg: OpenClawConfig): AgentKeyResolutionContext {
  return {
    cfg,
    defaultAgentId: resolveDefaultStoreAgentId(cfg),
    knownAgentIds: new Set(listConfiguredAgentIds(cfg)),
  };
}

function parseKnownAgentAlias(
  context: AgentKeyResolutionContext,
  key: string,
  opts?: { allowDefaultAgent?: boolean },
): KnownAgentAlias | null {
  ...
  const hasAgentPrefix = parts[0] === "agent";
  const agentId = normalizeAgentId(hasAgentPrefix ? parts[1] : parts[0]);
  if (!agentId || !context.knownAgentIds.has(agentId)) {
    return null;
  }
  if (opts?.allowDefaultAgent !== true && agentId === context.defaultAgentId) {
    return null;
  }

  return {
    agentId,
    rest: (hasAgentPrefix ? parts.slice(2) : parts.slice(1)).join(":"),
  };
}

function canonicalizeKnownAgentAlias(
  context: AgentKeyResolutionContext,
  alias: KnownAgentAlias,
): string {
  const agentId = normalizeAgentId(alias.agentId);
  const rest = alias.rest.trim().toLowerCase();
  if (!rest) {
    return resolveAgentMainSessionKey({ cfg: context.cfg, agentId });
  }
...
function canonicalizeSessionKeyForStore(params: {
  context: AgentKeyResolutionContext;
  storeAgentId: string;
  key: string;
}): string | null {
  ...
  const storeAgentId = normalizeAgentId(params.storeAgentId);
  const parsed = parseAgentSessionKey(raw);
  if (parsed) {
    const parsedAgentId = normalizeAgentId(parsed.agentId);
    if (parsedAgentId === storeAgentId && parsedAgentId === params.context.defaultAgentId) {
      const nestedAlias = parseKnownAgentAlias(params.context, parsed.rest);
      if (nestedAlias) {
        return canonicalizeKnownAgentAlias(params.context, nestedAlias);
      }
    }
    return canonicalizeMainSessionAlias({
  ...
  if (rawLower.startsWith("agent:")) {
    const malformedAlias = parseKnownAgentAlias(params.context, rawLower, {
      allowDefaultAgent: true,
    });
    return malformedAlias ? canonicalizeKnownAgentAlias(params.context, malformedAlias) : null;
  }

  const rawMainKey = normalizeMainKey(params.context.cfg.session?.mainKey);
  if (rawLower === "main" || rawLower === rawMainKey) {
    return resolveAgentMainSessionKey({ cfg: params.context.cfg, agentId: storeAgentId });
  }

  const scopedAlias = parseKnownAgentAlias(params.context, rawLower);
  if (scopedAlias) {
    return canonicalizeKnownAgentAlias(params.context, scopedAlias);
  }

  return canonicalizeSessionKeyForAgent(storeAgentId, rawLower);
}
...
export function resolveSessionStoreKey(params: {
  ...
  const context = createAgentKeyResolutionContext(params.cfg);
  ...
  if (parsed) {
    ...
    if (agentId === context.defaultAgentId) {
      const nestedAlias = parseKnownAgentAlias(context, parsed.rest);
      if (nestedAlias) {
        return canonicalizeKnownAgentAlias(context, nestedAlias);
      }
    }
  ...
  const scopedAlias = parseKnownAgentAlias(context, lowered);
  if (scopedAlias) {
    return canonicalizeKnownAgentAlias(context, scopedAlias);
  }
  if (lowered.startsWith("agent:")) {
    const malformedAlias = parseKnownAgentAlias(context, lowered, {
      allowDefaultAgent: true,
    });
    if (malformedAlias) {
      return canonicalizeKnownAgentAlias(context, malformedAlias);
    }
  }
  return canonicalizeSessionKeyForAgent(context.defaultAgentId, lowered);
}
...
export function loadCombinedSessionStoreForGateway(cfg: OpenClawConfig): {
  ...
  const context = createAgentKeyResolutionContext(cfg);
  ...
  if (storeConfig && !isStorePathTemplate(storeConfig)) {
    ...
    for (const [key, entry] of Object.entries(store)) {
      const canonicalKey = canonicalizeSessionKeyForStore({
        context,
