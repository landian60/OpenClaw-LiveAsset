...
function injectControlUiBasePath(body: string, basePathOverride: string | null): string {
  if (basePathOverride === null) {
    return body;
  }
  const script = `<script>window.__OPENCLAW_CONTROL_UI_BASE_PATH__=${JSON.stringify(basePathOverride)};</script>`;
...
function serveResolvedIndexHtml(res: ServerResponse, body: string, opts?: { basePathOverride?: string | null }) {
  res.setHeader("Content-Type", "text/html; charset=utf-8");
  res.setHeader("Cache-Control", "no-cache");
  res.end(injectControlUiBasePath(body, opts?.basePathOverride ?? null));
}
...
export function handleControlUiHttpRequest(
  ...
  const indexBasePathOverride =
    !basePath && (pathname === "/webchat" || pathname === "/webchat/" || pathname.startsWith("/webchat/"))
      ? "/"
      : null;
    ...
    try {
      ...
      if (path.basename(safeFile.path) === "index.html") {
        serveResolvedIndexHtml(res, fs.readFileSync(safeFile.fd, "utf8"), {
          basePathOverride: indexBasePathOverride,
        });
        return true;
      }
