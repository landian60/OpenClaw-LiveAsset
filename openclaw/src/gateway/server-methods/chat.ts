  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    try {
      ...
      void dispatchInboundMessage({
        ...
        .then(() => {
          if (agentRunStarted) {
            // Agent lifecycle final can arrive before assistant_response rewrites
            // are visible in the persisted transcript. Emit one more message-less
            // final after dispatch completion so webchat reloads the settled history.
            broadcastChatFinal({
              context,
              runId: clientRunId,
              sessionKey: rawSessionKey,
