...
describe("chat directive tag stripping for non-streaming final payloads", () => {
  ...
  it("emits a message-less final after agent-run completion so clients reload persisted history", async () => {
    createTranscriptFixture("openclaw-chat-send-agent-run-final-refresh-");
    mockState.finalText = "rewritten later";
    mockState.triggerAgentRunStart = true;
    mockState.agentRunId = "run-refresh";
    const respond = vi.fn();
    const context = createChatContext();
    ...
    const payload = await runNonStreamingChatSend({
      context,
      respond,
      idempotencyKey: "idem-agent-run-final-refresh",
    });

    expect(payload).toMatchObject({
      runId: "idem-agent-run-final-refresh",
      sessionKey: "main",
      state: "final",
    });
