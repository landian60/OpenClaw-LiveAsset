...
describe("gateway session utils", () => {
  ...
  test("resolveSessionStoreKey canonicalizes bare keys to default agent", () => {
    const cfg = {
      session: { mainKey: "main" },
      agents: { list: [{ id: "ops", default: true }, { id: "zhn" }, { id: "new-agent" }] },
    } as OpenClawConfig;
    ...
    expect(resolveSessionStoreKey({ cfg, sessionKey: "new-agent:main" })).toBe(
      "agent:new-agent:main",
    );
    expect(resolveSessionStoreKey({ cfg, sessionKey: "zhn" })).toBe("agent:zhn:main");
    expect(resolveSessionStoreKey({ cfg, sessionKey: "agent:zhn" })).toBe("agent:zhn:main");
    expect(resolveSessionStoreKey({ cfg, sessionKey: "agent:ops:zhn" })).toBe("agent:zhn:main");
  });
...
describe("loadCombinedSessionStoreForGateway includes disk-only agents (#32804)", () => {
  ...
  test("repairs polluted cross-agent keys and hides malformed variants", async () => {
    await withStateDirEnv("openclaw-agent-session-repair-", async ({ stateDir }) => {
      const agentsDir = path.join(stateDir, "agents");
      const mainDir = path.join(agentsDir, "main", "sessions");
      const newAgentDir = path.join(agentsDir, "new-agent", "sessions");
      const zhnDir = path.join(agentsDir, "zhn", "sessions");
      fs.mkdirSync(mainDir, { recursive: true });
      fs.mkdirSync(newAgentDir, { recursive: true });
      fs.mkdirSync(zhnDir, { recursive: true });

      fs.writeFileSync(
        path.join(mainDir, "sessions.json"),
        JSON.stringify({
          "agent:main:new-agent:main": { sessionId: "s-new-polluted", updatedAt: 300 },
          "agent:zhn": { sessionId: "s-zhn-malformed", updatedAt: 250 },
          "agent:main:zhn": { sessionId: "s-zhn-polluted", updatedAt: 400 },
        }),
        "utf8",
      );
      fs.writeFileSync(
        path.join(newAgentDir, "sessions.json"),
        JSON.stringify({
          "agent:new-agent:main": { sessionId: "s-new-existing", updatedAt: 100 },
        }),
        "utf8",
      );
      fs.writeFileSync(path.join(zhnDir, "sessions.json"), JSON.stringify({}), "utf8");
      ...
      const cfg = {
        ...
        agents: {
          list: [{ id: "main", default: true }, { id: "new-agent" }, { id: "zhn" }],
        },
      } as OpenClawConfig;

      const { store } = loadCombinedSessionStoreForGateway(cfg);
      expect(store["agent:new-agent:main"]?.sessionId).toBe("s-new-polluted");
      expect(store["agent:zhn:main"]?.sessionId).toBe("s-zhn-polluted");
      expect(store["agent:main:new-agent:main"]).toBeUndefined();
      expect(store["agent:main:zhn"]).toBeUndefined();
      expect(store["agent:zhn"]).toBeUndefined();
    });
  });
});
