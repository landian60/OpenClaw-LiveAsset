...
describe("GatewayClient", () => {
  ...
  it("injects root base-path override for /webchat SPA entrypoints", async () => {
    await withControlUiRoot({ indexHtml: "<html><head></head><body></body></html>\n" }, async (tmp) => {
      const { res } = makeControlUiResponse();
      const handled = handleControlUiHttpRequest(
        { url: "/webchat/chat", method: "GET" } as IncomingMessage,
        res,
      ...
      expect(res.end).toHaveBeenCalledWith(
        expect.stringContaining('window.__OPENCLAW_CONTROL_UI_BASE_PATH__="/"'),
      );
    });
  });
